...
def infix_to_postfix(expression, trace=None):
    """Shutting yard. Returns the postfix expression as a string.

    if 'trace' is a list, one row per token is appended to it.
    """
    output = []
    ...
    for token in tokenize(expression):
        if token in PRECEDENCE:
            ...
            #check if there operators inside the parethesis
...
def evaluate_postfix(expression, trace=None):
    ...
    for token in tokenize(expression):
        ...
        if trace is not None:
            trace.append((token, " ".join(convert_and_evaluate(v) for v in values._items)))
    if values.size() != 1:
...
def convert_and_evaluate(value):
