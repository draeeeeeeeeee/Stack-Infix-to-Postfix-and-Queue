...
class CircularQueue:
    ...
    def slots(self):
        ...
        For inspecting wraparound during the demonstration.May pepe si zandrae Not part of the
...
class Deque:
    ...
    def remove_front(self):
        ...
        if self.is_empty():
            raise IndexError("remoce_front from an empty dequeue")
        return self._items.pop(0)

    def remove_rear(self):
        """Step 12. Remove and return the last item. IndexError when empty."""
        if self.is_empty():
            raise IndexError("remoce_rear from an empty dequeue")
...
def is_palindrome(text):
    ...
    for character in text:
        if character.isalpha():
            letters.add_rear(character.lower())
